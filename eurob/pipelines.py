...
class EurobPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('eurob.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `eurob`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM eurob WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO eurob (date, title, link, content)"
